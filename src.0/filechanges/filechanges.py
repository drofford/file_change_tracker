...
# set up logging
#
level = ZZLOG.DEBUG if os.getenv("DEBUG", "") == "Y" else ZZLOG.INFO
ZZLOG.basicConfig(format="%(asctime)s %(levelname)s : %(message)s", level=level)
logger = ZZLOG.getLogger()
...
def corecursor(conn: sqlite3.Connection, query: str, args: list = None, hits:list=None) -> bool:
    debug(f'query = "{query}"')
...
def md5indb(fname: str, table: str = FILE_TABLE_NAME):
...
def main():
    ...
    # connect to database, creating if necessary

    # determine list of files in monitored subdirs

    # process list of files, checking for changes
